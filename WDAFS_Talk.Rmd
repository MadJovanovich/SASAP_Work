...
BB QA ISSUE: NEED TO FLAG/ADD NOTE, AND POSSIBLY EXCLUDE 2017 FROM DATA SET 

NEED TO PLOT # GOALS BY SPECIES, FACET BY REGION 


Begin presentation with the "integrated approach": what needed to be done to get to this point? 
...
* Lots of data QA still necessary
  - see Bristol Bay achievement plot
  - statewide plot: why aren't all goals appearing? 
* Definite consistent color schemes for different types of data visualizations
* Ensure aspect ratios are appropriate for viewing
...
achievement_colors <- c(Unmet = "#D73027", Met = "#A6D968", Exceeded = "#3288BD")
...
Show there are no goals in the Arctic by adding a region to the data set: *this will be displayed in the table that shows goal by region*
...
```{r Subsetting for Run Timing, eval=FALSE}

BB_esc <- subset(escapement_daily, SASAP.Region == "Bristol Bay")
BB_sockeye <- subset(BB_esc, Species == "sockeye")

ggplot(BB_sockeye, aes(x=sampleDate, y = DailyCount)) +
  geom_point() + 
  scale_x_discrete(breaks = pretty_breaks(n = 100))

#ave_RunTiming <- BB_sockeye %>%
```

```{r QA Data, eval=FALSE}
BB_sockeye_Cunningham <- subset(cunningham_data, Species == "sockeye") %>%
  arrange(desc(sampleDate))

plottingdf$sampleYear <- as.numeric(plottingdf$sampleYear)
range(plottingdf$sampleYear, na.rm=T)

#How can I write a loop based on unique SASAP.Region observations, instead of always subsetting individually? 
APAL <- subset(esc_goals_results, SASAP.Region == "Alaska Peninsula and Aleutian Islands")
BB <- subset(esc_goals_results, SASAP.Region == "Bristol Bay")
CH <- subset(esc_goals_results, SASAP.Region == "Chignik")
CI <- subset(esc_goals_results, SASAP.Region == "Cook Inlet")
CR <- subset(esc_goals_results, SASAP.Region == "Copper River")
KOD <- subset(esc_goals_results, SASAP.Region == "Kodiak")
KOTZ <- subset(esc_goals_results, SASAP.Region == "Kotzebue")
KUSK <- subset(esc_goals_results, SASAP.Region == "Kuskokwim")
NS <- subset(esc_goals_results, SASAP.Region == "Norton Sound")
PWS <- subset(esc_goals_results, SASAP.Region == "Prince William Sound")
SE <- subset(esc_goals_results, SASAP.Region == "Southeast")
YUK <- subset(esc_goals_results, SASAP.Region == "Yukon")

range(APAL$sampleYear)
range(BB$sampleYear) #includes 2017
range(CH$sampleYear)
range(CI$sampleYear)
range(CR$sampleYear)
range(KOD$sampleYear)
range(KOTZ$sampleYear)
range(KUSK$sampleYear)
range(NS$sampleYear)
range(PWS$sampleYear)
range(SE$sampleYear)
range(YUK$sampleYear)
#Bristol Bay is the only region that includes 2017, so let's remove it (causing issues anyways)
#See: Establishing Proportions of Achievement, for removal of all data sampleYear > 2017 
```
...
#that way, esc_goals_results would retains NAs, percents would omit them

esc_goals_results <- bind_rows(escapement_goals, Arctic) %>%
  arrange(SASAP.Region, LocationID)

esc_goals_results <- esc_goals_results %>%
...
esc_goals_results$SASAP.Region[which(esc_goals_results$SASAP.Region == "Alaska Peninsula and Aleutian Islands")] <- "AK Pen. and Aleutians"
...
[Sustaining Alaska's Fisheries](https://www.adfg.alaska.gov/static/fishing/PDFs/50years_cf/sustain_wild2000s.pdf)
  ...
  **  * SUstainable Salmon Policy Established in 2000, followed by stamp of approcal from the Marine Stewardship Council 
...
Where are BEGs? Why aren't all goals appearing in this plot? Besides that, the design of this plot looks good. 
...
ggplot(data = plottingdf, aes(x = SASAP.Region, fill=factor(types), y=(..count..))) + 
    ...
    scale_fill_brewer("Colors in Spectral", palette="Spectral", breaks = c("agreement", "BEG", "inRiver", "MT", "OEG", "SEG")) 
```

Since theme_hc() doesn't look great when faceting, maybe I should create my own theme: 

* remove vertical lines, retain horizontal
* no background on facet titles 
* legend.position = "right"
...
ggplot(data = plottingdf, aes(x = sampleYear, fill=factor(types), y=(..count..))) + 
    ...
    labs(x = "Year",
         y = "Number of Goals", 
         title = "Escapement Goals by Region") +
    scale_y_continuous(labels = formatter(nsmall = 0)) +
...
#WHY IS KOTZEBUE STILL GIVING ME ISSUES?!
```
...
```
```{r QA AK Pen and Aleutians, eval=FALSE}
...
BB_sockeye <- subset(BBpercents, Species == "sockeye") %>% arrange(sampleYear)
BB_chum <- subset(BBpercents, Species == "chum") %>% arrange(sampleYear)
BB_chinook <- subset(BBpercents, Species == "chinook") %>% arrange(sampleYear)
BB_pink <- subset(BBpercents, Species == "pink") %>% arrange(sampleYear)
BB_coho <- subset(BBpercents, Species == "coho") %>% arrange(sampleYear)

BB_sock <- subset(esc_goals_results, Species == "sockeye") %>% arrange(sampleYear)

BBsock <- subset(escapement_goals, SASAP.Region == "Bristol Bay" & Species == "sockeye") %>% arrange(desc(sampleYear))
#I've found the issue!! Within the original df. Need to talk to Jeanette about this! 
```
```{r QA Chignik}
