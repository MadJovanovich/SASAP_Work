...
The name of this region included in the original file was too long to fit into any of the plots, so here it is abbreviated. 
...
* Exceeded/met/unmet `scale_fill_manual(values = "achievement_colors")` *continual work in progress* 
...
```{r QA Data - Year Ranges and Complete Data, include=FALSE}
...
kable(YearQA, caption = "Ranges of Year in Original Dataset")
```

Notice that not all regions have the same max year. Should I only evaluate thorough 2014? Seems rather silly given that would exclude all of Bristol Bay's most recent large runs, as well as the past couple Kusko years of slight improvement. 

```{r kable showing Year QA}
kable(escapement_goals_QAedkable, caption = "Ranges of Years after removing 2017 incomplete data")
```
